...
type UbahState = {
  modal: boolean;
  data?: RujukanInternal;
};
type UbahAction = { type: "setUbah"; ubah: UbahState };

export default function RujukanInternal() {
  ...
  const [dataList, setDataList] = useState<RujukanInternal[]>([]);
  ...
  const handleHapus = async () => {
    try {
      // const hapusPasien = await fetch(
      //   `${APIURL}/rs/rujukan/internal/${hapus.data?.id}`,
      //   {
      //     method: "DELETE",
      //     headers: headers,
      //   }
  ...
  const loadData = async (signal?: AbortSignal) => {
    try {
      setIsMutating(true);
      const url = new URL(`${APIURL}/rs/rujukan/internal`);
      const params = {
        page: meta.page,
                  ...
                  [...Array(15)].map((_, i) => (
                    ...
                    >
                      <td className="h-[56.5px]">
                        <p className="mx-auto h-[32px] w-48 rounded-sm bg-slate-200 dark:bg-slate-400"></p>
                      </td>
                      <td>
                  ...
                  dataList?.map((data, i) => (
                    ...
                    >
                      <td className="border-b border-slate-200 dark:border-gray-700">
                        <p
                          className={cn(
                            "mx-auto w-48 rounded-sm py-1.5 text-center font-light tracking-wide text-slate-100",
                            "bg-green-500"
                          )}
                        >
                          {data.no_rujuk}
                      ...
                      <td className="border-b border-slate-200 dark:border-gray-700">
                        <div className="flex flex-nowrap items-center justify-center gap-1">
                          ...
                          {data.id_asuransi === 0 && data.antrian !== null ? (
                            <Tooltip.Provider
                              delayDuration={300}
                              disableHoverableContent
                            >
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-slate-700">
                  ...
                  >
                    Hapus Rujukan Internal pasien {hapus.data?.nama}?
                  </Dialog.Title>
