...
export default function PengeluaranObat() {
  ...
  const loadData = async (signal?: AbortSignal) => {
    try {
      setIsMutating(true);
      const url = new URL(`${APIURL}/rs/farmasi/pengeluaran`);
      const params = {
        page: meta.page,
...
const PengeluaranDialog = ({
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(lihat.modal ? "Lihat Pengeluaran" : "Tambah Pengeluaran");
    return lihat.modal ? "Lihat Pengeluaran" : "Tambah Pengeluaran";
  ...
  const [pengeluaran, setPengeluaran] = useState<HasilTransaksiBarang>();
  ...
  useEffect(() => {
    ...
    setValue("id_jenis", pengeluaran.id_jenis);
    setValue("id_depo", pengeluaran.id_depo);
    setValue("id_ref", pengeluaran.id_ref);
    setValue("keterangan", pengeluaran.keterangan);
    setValue(
      "detail",
  ...
  useEffect(() => {
    ...
    if (lihat.modal) loadPengeluaran();
  ...
  const submitHandler: SubmitHandler<TransaksiBarangTSchema> = async (
    ...
    try {
      e?.preventDefault();
      // if (ubah.modal) {
      //   const put = await fetch(`${APIURL}/rs/farmasi/pengeluaran/${ubah.data?.id}`, {
      //     method: "PUT",
      //     headers: headers,
      ...
      // } else {
      const post = await fetch(`${APIURL}/rs/farmasi/pengeluaran`, {
        method: "POST",
        headers: headers,
        body: JSON.stringify(data),
      });
