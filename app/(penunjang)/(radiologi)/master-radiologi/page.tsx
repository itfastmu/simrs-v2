...
export default function ListRadiologi() {
  ...
  const handleHapus = async () => {
    try {
      const url = !!hapus.data?.id_penunjang
        ? `${APIURL}/rs/radiologi/format/${hapus.data?.id}`
        : `${APIURL}/rs/radiologi/${hapus.data?.id}`;
  ...
  const loadGrouping = async () => {
    try {
      setIsMutating(true);
      const url = new URL(`${APIURL}/rs/radiologi/group`);
      const params = {
        page: meta.page,
  ...
  const [listRadiologi, setListRadiologi] = useState<Penunjang[]>([]);
  const loadRadiologi = async () => {
    try {
      setIsMutating(true);
      const url = new URL(`${APIURL}/rs/radiologi/master/detail`);
      const params = {
        page: meta.page,
        perPage: meta.perPage,
        // cari: deferredCari,
                      ...
                      {isMutating ? (
                        ...
                        listFormat?.map((data, i) => (
                          <tr
                            ...
                            key={i}
                          >
                            <td className="border-b border-slate-200 p-2 align-top dark:border-gray-700">
                            ...
                            <td className="border-b border-slate-200 pt-4 align-top dark:border-gray-700">
                              <div className="flex items-center justify-center gap-2">
                                <Tooltip.Provider
                                  delayDuration={300}
                                  disableHoverableContent
...
const RadiologiDialog = ({
  ...
  // const [listJenis] = useState(["Radiologi"]);
  ...
  const [pemRadOptions, setPemRadOptions] = useState<MyOptions>([]);
  const loadPemRad = async (inputText: string) => {
    try {
      const url = new URL(`${APIURL}/rs/radiologi/master/detail`);
      const params = {
        perPage: 50,
  ...
  useEffect(() => {
    ...
    if (judul === "Format") {
      ...
      // setValue("jenis", ubah.data?.jenis!);
    }
  ...
  const submitHandler: SubmitHandler<FormSchemaType> = async (data, e) => {
    try {
      ...
      if (ubah.modal) {
        ...
        const url =
          judul === "Format"
            ...
            : `${APIURL}/rs/radiologi/${ubah.data?.id}`;
        ...
        const { type, ...parsedData } = data;
        if (type === "Pemeriksaan" && "id_jenis" in parsedData)
          delete parsedData.id_jenis;
        const url =
          judul === "Format"
            ? `${APIURL}/rs/penunjang/format`
            : `${APIURL}/rs/radiologi/1`;
      ...
      tutup();
      judul === "Format" ? loadFormat() : loadRadiologi();
    } catch (err) {
      const error = err as Error;
      toast.error(error.message);
      console.error(error);
                  ...
                  >
                    {judul === "Format" ? (
                            ...
                            render={({ field: { onChange, value } }) => {
                              return (
                                <AsyncSelectInput
                                  ...
                                  menuPosition="fixed"
                                  defaultOptions={pemRadOptions}
                                  loadOptions={loadPemRad}
                                  value={pemRadOptions?.find(
                                    (val) => val.value === value
                                  )}
                      ...
                      <div className="flex gap-2">
                        ...
                        >
                          {"jenis" in errors && errors.jenis ? (
                            <p className="p-0.5 text-xs text-red-500">
                              {"jenis" in errors && errors.jenis.message}
                            </p>
                          ) : null}
                          <Controller
                            control={control}
                            name="jenis"
                            render={({ field: { onChange, value } }) => {
                              const options = listJenis.map((val) => ({
                                value: val,
                                label: val,
                              }));
