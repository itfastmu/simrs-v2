...
export default function PangkatDialog({
  pangkatDialog,
  setPangkatDialog,
}: {
  pangkatDialog: boolean;
  setPangkatDialog: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  const tutup = () => {
    setPangkatDialog(false);
  };
  ...
  const handleHapus = async () => {
    try {
      const hapusKlinik = await fetch(
        `${APIURL}/rs/pangkat/${hapus.data?.id}`,
        {
          method: "DELETE",
          headers: headers,
        }
  ...
  useEffect(() => {
    if (pangkatDialog) loadData();
  }, [meta.page, meta.perPage, deferredCari, pangkatDialog]);

  const tableDivRef = useRef<HTMLDivElement>(null);
  return (
    <Transition show={pangkatDialog} as={Fragment}>
...
const FormPangkatDialog = ({
  tambahDialog,
  setTambahDialog,
  ubah,
  ubahDispatch,
  loadData,
}: FormPangkatDialogProps) => {
  const tutup = () => {
    reset();
    tambahDialog
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(ubah.modal ? "Ubah Pangkat" : "Tambah Pangkat");
    return ubah.modal ? "Ubah Pangkat" : "Tambah Pangkat";
  ...
  const submitHandler: SubmitHandler<Pangkat> = async (data, e) => {
    try {
      e?.preventDefault();
      if (ubah.modal) {
        const put = await fetch(`${APIURL}/rs/pangkat/${ubah.data?.id}`, {
          method: "PUT",
          headers: headers,
        ...
        const post = await fetch(`${APIURL}/rs/pangkat`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify(data),
        });
                ...
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-slate-700">
                  ...
                  <form className="mt-2" onSubmit={handleSubmit(submitHandler)}>
                    ...
                    >
                      ...
                      <Input placeholder="Nama Pangkat" {...register("nama")} />
                    </div>
                    <div className="mt-4 flex justify-end gap-1">
                      <Button
                        type="submit"
                        color={
                          judul === "Tambah Pangkat" ? "green100" : "cyan100"
                        }
                      >
                        {judul}
                      </Button>
