...
export default function JabatanDialog({
  ...
  jabatanDialog: boolean;
  setJabatanDialog: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  const tutup = () => {
    setJabatanDialog(false);
  };
  ...
  const handleHapus = async () => {
    try {
      const hapusKlinik = await fetch(
        `${APIURL}/rs/jabatan/${hapus.data?.id}`,
        {
          method: "DELETE",
          headers: headers,
        }
  ...
  useEffect(() => {
    if (jabatanDialog) loadData();
  }, [meta.page, meta.perPage, deferredCari, jabatanDialog]);

  const tableDivRef = useRef<HTMLDivElement>(null);

  return (
    <Transition show={jabatanDialog} as={Fragment}>
...
const FormJabatanDialog = ({
  tambahDialog,
  setTambahDialog,
  ubah,
  ubahDispatch,
  loadData,
}: FormJabatanDialogProps) => {
  const tutup = () => {
    reset();
    tambahDialog
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(ubah.modal ? "Ubah Jabatan" : "Tambah Jabatan");
    return ubah.modal ? "Ubah Jabatan" : "Tambah Jabatan";
  ...
  type Jabatan = z.infer<typeof JabatanSchema>;
  ...
  const submitHandler: SubmitHandler<Jabatan> = async (data, e) => {
    try {
      e?.preventDefault();
      if (ubah.modal) {
        const put = await fetch(`${APIURL}/rs/jabatan/${ubah.data?.id}`, {
          method: "PUT",
          headers: headers,
        ...
        const post = await fetch(`${APIURL}/rs/jabatan`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify(data),
        });
                ...
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-slate-700">
                  ...
                  <form className="mt-2" onSubmit={handleSubmit(submitHandler)}>
                    ...
                    >
                      ...
                      <Input placeholder="Nama Jabatan" {...register("nama")} />
                    </div>
                    <div className="mt-4 flex justify-end gap-1">
                      <Button
                        type="submit"
                        color={
                          judul === "Tambah Jabatan" ? "green100" : "cyan100"
                        }
                      >
                        {judul}
                      </Button>
