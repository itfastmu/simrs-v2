  ...
  const handleHapus = async () => {
    try {
      const hapusKlinik = await fetch(
        `${APIURL}/rs/pendidikan/${hapus.data?.id}`,
        {
          method: "DELETE",
          headers: headers,
        }
...
const PendidikanDialog = ({
  tambahDialog,
  setTambahDialog,
  ubah,
  ubahDispatch,
  loadData,
}: PendidikanDialogProps) => {
  const tutup = () => {
    reset();
    tambahDialog
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(ubah.modal ? "Ubah Pendidikan" : "Tambah Pendidikan");
    return ubah.modal ? "Ubah Pendidikan" : "Tambah Pendidikan";
  ...
  type Pendidikan = z.infer<typeof KlinikSchema>;
  ...
  const submitHandler: SubmitHandler<Pendidikan> = async (data, e) => {
    try {
      e?.preventDefault();
      if (ubah.modal) {
        const put = await fetch(`${APIURL}/rs/pendidikan/${ubah.data?.id}`, {
          method: "PUT",
          headers: headers,
        ...
        const post = await fetch(`${APIURL}/rs/pendidikan`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify(data),
        });
                  ...
                  <form className="mt-2" onSubmit={handleSubmit(submitHandler)}>
                    ...
                    <div className="mt-4 flex justify-end gap-1">
                      <Button
                        type="submit"
                        color={
                          judul === "Tambah Pendidikan" ? "green100" : "cyan100"
                        }
                      >
                        {judul}
                      </Button>
