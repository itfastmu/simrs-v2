...
export default function CaraBayar() {
  ...
  const [dataList, setDataList] = useState<CaraBayar[]>([]);
  ...
  const handleHapus = async () => {
    try {
      const resp = await fetch(`${APIURL}/rs/asuransi/${hapus.data?.id}`, {
        method: "DELETE",
        headers: headers,
      });
...
const CaraBayarDialog = ({
  tambahDialog,
  setTambahDialog,
  ubah,
  ubahDispatch,
  loadData,
}: CaraBayarDialogProps) => {
  const tutup = () => {
    reset();
    tambahDialog
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(ubah.modal ? "Ubah Cara Bayar" : "Tambah Cara Bayar");
    return ubah.modal ? "Ubah Cara Bayar" : "Tambah Cara Bayar";
  }, [tambahDialog, ubah.modal]);

  const CaraBayarSchema = z.object({
    // id: z.number(),
    nama: z.string().min(1, "harus diisi"),
    // aktif: z.boolean(),
  });

  type CaraBayar = z.infer<typeof CaraBayarSchema>;
  ...
  const {
    ...
    resolver: zodResolver(CaraBayarSchema),
    // defaultValues: {
    //   aktif: true,
    // },
  });
  ...
  const submitHandler: SubmitHandler<CaraBayar> = async (data, e) => {
    try {
      e?.preventDefault();
      let resp;
      if (ubah.modal) {
        const put = await fetch(`${APIURL}/rs/asuransi/${ubah.data?.id}`, {
          method: "PUT",
          headers: headers,
        ...
        const post = await fetch(`${APIURL}/rs/asuransi`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify(data),
        });
                  ...
                  <form className="mt-2" onSubmit={handleSubmit(submitHandler)}>
                    ...
                    <div className="mt-4 flex justify-end gap-1">
                      <Button
                        type="submit"
                        color={
                          judul === "Tambah Cara Bayar" ? "green100" : "cyan100"
                        }
                      >
                        {judul}
                      </Button>
