...
export default function Unit() {
  ...
  const [dataList, setDataList] = useState<Unit[]>([]);
...
const UnitDialog = ({
  ...
  const judul = useMemo(() => {
    ...
    setJudulLama(ubah.modal ? "Ubah Unit" : "Tambah Unit");
    return ubah.modal ? "Ubah Unit" : "Tambah Unit";
  ...
  const submitHandler: SubmitHandler<Unit> = async (data, e) => {
    try {
      e?.preventDefault();
      if (ubah.modal) {
        const put = await fetch(`${APIURL}/rs/unit/${ubah.data?.id}`, {
          method: "PUT",
          headers: headers,
        ...
        const post = await fetch(`${APIURL}/rs/unit`, {
          method: "POST",
          headers: headers,
          body: JSON.stringify(data),
        });
                ...
                <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all dark:bg-slate-700">
                  ...
                  <form className="mt-2" onSubmit={handleSubmit(submitHandler)}>
                    ...
                    >
                      ...
                      <Input placeholder="Nama Unit" {...register("nama")} />
                    </div>
                    <div className="mt-4 flex justify-end gap-1">
                      <Button
                        type="submit"
                        color={judul === "Tambah Unit" ? "green100" : "cyan100"}
                      >
                        {judul}
                      </Button>
